...
namespace ServiceCenter.Database
{
    public class DBClass
    {
        ...
        public void connect()
        {
            string path = "Data Source=.\\DB\\dom_ru.db";
        ...
        public List<Staff> getMasterList()
        {
            ...
            connect();
            cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT * FROM Staff WHERE role = 'Техник'";
            reader = cmd.ExecuteReader();
            if (reader.HasRows)
            {
                while (reader.Read())
        ...
        {
            Order order = new Order(id, 0, "", new List<Service>(0));
            SqliteDataReader reader2;
            connect();
            cmd = conn.CreateCommand();
            ...
            if (reader2.HasRows)
            {
                while (reader2.Read())
                {
                    ...
                    order.address = reader2.GetValue(2).ToString();
                }
            }
            reader2.Close();
            conn.Close();
        ...
        public Application getApp(int id)
        {
            Application application = new Application(id, new DateTime(2000, 1, 1), new Client(0, "", "", "", "", ""),
                "", new Staff(0, "", "", ""), new Order(0, 0, "", new List<Service>(0)));
            int client_id = 0;
            int staff_id = 0;
            int order_id = 0;
            connect();
        ...
        public int addOrder(Order order)
        {
            connect();
            cmd = conn.CreateCommand();
            cmd.CommandText = $"INSERT INTO Orders (Cost, Address) VALUES ({order.cost}, " +
               $"'{order.address}')";
        ...
        public void editOrder(Order order)
        {
            connect();
            cmd = conn.CreateCommand();
            cmd.CommandText = $"UPDATE Orders SET Cost = {order.cost}, Address = '{order.address}' WHERE id = {order.id}";
            cmd.ExecuteNonQuery();
            close();
        }
