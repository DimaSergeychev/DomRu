...
namespace ServiceCenter.GUI
{
    public partial class AppCreateF : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (checkInputs())
            {
                if (MessageBox.Show("Вы хотите создать заявку с внесёнными данными?", "Подтверждение", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    Order order = new Order(-1, float.Parse(richTextBox4.Text.Replace(" руб.", "")), richTextBox5.Text, services);
        ...
        {
            if (MessageBox.Show("Вы действительно хотите выйти без сохранения  заявки?", "Подтверждение", MessageBoxButtons.YesNo) == DialogResult.Yes);
